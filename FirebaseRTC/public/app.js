...
let roomId = null;
let exists2 = false;
let sendchannel2 = null; //RTCdatachaneel for the local 
let sendchannel3 = null; //Rtcdatachanel for the local
let receiveChannel2 = null; //RTCdatachaneel for the remoteA
let receiveChannel3 = null; //RTCdatachanel for the remoteB
...
async function createRoom() {
  ...
  // 1과 2, 1과 3 사이의 RTCPeerConnection, Data channel 생성
  ...
  sendchannel2 = peerConnection12.createDataChannel("sendChannel1to2");
  sendchannel2.onopen = () => {
        console.log("Datachannel open");
  };
  sendchannel2.onclose = () => {
        consloe.log("Datachannel close");
  };
  ...
  sendchannel3 = peerConnection31.createDataChannel("sendChannel1to3");
  sendchannel3.onopen = () => {
        console.log("Datachannel open");
  };
  sendchannel3.onclose = () => {
        consloe.log("Datachannel close");
  };
...
async function joinRoomById(roomId) {
  ...
  if (roomSnapshot.exists) {
    ...
    //setuptheremotepeer
    peerConnection23.ondatachannel = receiveChannelCallback;
...
function receiveChannelCallback(event) {
  ...
  receiveChannel3 = event.channel;
  receiveChannel2.onopen =  () => {
    console.log("Datachannel open");
  };
  receiveChannel2.onclose = () => {
    consloe.log("Datachannel close");
  };
  receiveChannel3.onopen =  () => {
    console.log("Datachannel open");
  };
  receiveChannel3.onclose = () => {
    consloe.log("Datachannel close");
  };
}
//handling channel status change 
...
async function playgame(peerConnection) {
 ...
 //user1은 w1, user2는 w2, user3는 w3를 할당받음 
  ...
  const db = firebase.firestore();
  const wlist = await db.collection("rooms").doc("forbbidenword");
...
function sendwordlist2(w1,w3) {
    var message2 = w1,w3;
    sendChannel1to2.send(message2);
 }
