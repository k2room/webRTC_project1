...
const $btnMic = document.querySelector("#micBtn");
...
async function playgame(PeerConnection) {
  ...
  document.querySelector("#userDefinition").innerText =
  ...
  if (userID == 1) {
    ...
    document.querySelector("#topic").innerText =
      'The topic of the game is "GIST"';
    document.querySelector("#localWord").innerText = "Guess your's!";
    document.querySelector("#forbiddenword1").innerText =
      "Forbidden word: " + forbidden2;
    document.querySelector("#forbiddenword2").innerText =
      "Forbidden word: " + forbidden3;
    ...
    roomRefFlags.doc("userStates").set(flags);
  } else {
    // DB에서 word(금칙어) 불러오기
    const db = firebase.firestore();
    ...
    if (snapshot.data().word1 != undefined) {
      ...
      document.querySelector("#waitUser1ToPlay").innerText =
        "User1 did not distributed words. Press the button again.";
      document.querySelector("#playBtn").disabled = false;
    }
    }
  ...
  //sojeong
  roomRef.collection("flags").onSnapshot((snapshot) => {
    snapshot.docChanges().forEach(async (change) => {
      if (change.type === "modified") {
        ...
        if (userID == 1) {
          if (data.flag2 == false) {
            ...
            remoteStreamA.getAudioTracks()[0].enabled = false;
          } else if (data.flag3 == false) {
            remoteStreamB.getTracks().forEach((track) => track.stop());
            remoteStreamB.getAudioTracks()[0].enabled = false;
          }
        } else if (userID == 2) {
          if (data.flag1 == false) {
            remoteStreamA.getTracks().forEach((track) => track.stop());
